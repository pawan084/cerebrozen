...
- [docs/INCIDENT_RUNBOOK.md](docs/INCIDENT_RUNBOOK.md) — something is broken NOW: severity by user harm, what an outage does and does not do to the crisis path (the numbers are compiled into each client, so a total backend outage cannot take the crisis door down), per-failure playbooks, and the honest note that there is no alerting yet
...
- [docs/POSITIONING.md](docs/POSITIONING.md) — market positioning + the USP in four pillars, and the timing thesis: 8 US states now ban or regulate AI therapy and Woebot exited consumer, so "companion, never clinician" became a moat. Quotes market size as a **range** on purpose; keeps a "not true yet" panel. Pairs with INVESTOR_READINESS (benchmarks) and CLAIMS_MAP (the honesty mechanism it sells)
