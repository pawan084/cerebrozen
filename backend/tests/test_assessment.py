...
async def test_ai_topics_anchor_primary_selection(monkeypatch):
    """Even with LLM topics, the FIRST topic must be the curated anchor for the
    user's first selection (their explicit choice is visible without scrolling
    the starters rail)."""
    ...
    assert texts[0] == assessment._TOPIC_SEEDS["Confidence"][0]
