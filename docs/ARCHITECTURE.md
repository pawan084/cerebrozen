...
| Safety plan (`/safety-plan/me`) | `models/safety_plan.py` — the six Stanley-Brown sections, versioned, archive-not-delete; PUT merges unset fields so a guided flow can save one section at a time. **User-authored: there is no path for the model to persist a plan.** Never gates a crisis reply | iOS/Android/web guided authoring (pending); an absent plan is `null`, which every client must render as an invitation, not an error |
