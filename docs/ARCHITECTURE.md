...
| Turn safety markers on the `done` frame (`safety_flag`, **`pacing`**) | Engine | Android, apps/app | Additive; absent or `""` means "ordinary turn", so an older client is unaffected and a client must **fail closed on a value it does not recognise** (draw nothing, never guess). `pacing` (`""`\|`pause`\|`distress_route`, added 2026-07-21 for CHAT_SPEC §1.7) says which session-pacing intervention shaped this reply — `app/safety/pacing.py` injects it into the **system prompt**, so the reply comes back in the coach's own voice and a support-route turn is otherwise byte-indistinguishable from ordinary coaching prose. Vocabulary is the engine's `block_for` kind verbatim and matches the `cerebrozen_session_pacing_total{kind}` metric label. **It is PER-TURN, not session state:** `safety_node` (the graph entry point, and the only node a crisis turn is guaranteed to run) resets it and `_run_stage` rewrites it, because graph state is checkpointed and node returns merge — a marker written once and not rewritten would stay set for the rest of the session and paint every later reply. A crisis turn therefore always carries `pacing: ""`: a takeover is never also a pacing card. Earliest client that may rely on it: Android build of 2026-07-21 (`ui/screens/CoachScreen.kt::pacingOf`) |
