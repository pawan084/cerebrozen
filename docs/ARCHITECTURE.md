...
| Daily abuse ceilings (429) | `services/usage.py` — `CEILINGS` per feature, `DAILY_CEILING_CODE = "daily_ceiling"` in a STRUCTURED `detail` with `feature`, `limit` and `resets_at`. Deliberately distinguishable from `free_daily_limit`, which means "upgrade and this goes away"; this one means "come back tomorrow", and the ceilings are IDENTICAL on every tier because differing ones would be a pricing decision (see the banned-phrase table in CLAIMS_MAP). Counted in `daily_usage` by a single `INSERT … ON CONFLICT DO UPDATE … RETURNING`, because a read-then-write ceiling holds only under sequential load | web `DailyCeilingError` → its own message and reset time, and NOT the upgrade card the free cap shows ⇄ android `Session.DailyCeilingException`, carrying the same feature/limit/resets_at ⇄ iOS `APIError.dailyCeiling`. Every ceiling is 5–20× a heavy day, so a genuine user should never see one — and `GET /admin/metrics/ceilings` is how anyone can tell whether that held. It reports counts for everyone and identifiers only for accounts that actually reached a ceiling, and says `alerting: false` in the payload because nobody is paged |
| Email-verification gate (403) | `services/verification.py` — `UNVERIFIED_CODE = "email_unverified"` plus a `feature` name in a STRUCTURED `detail`, so a client can say which thing is waiting instead of showing a generic wall. Applies to voice, plans, goal decomposition, assessment and oracle; never to chat, and never to reading or exporting your own data | web `VerificationRequiredError` carries the feature name, and `/auth/me.email_verification_required` drives a `<VerifyEmailNotice>` on the settings page so the prompt arrives BEFORE the 403 rather than as it. ⇄ android `Session.VerificationRequiredException` + a `VerifyEmailCard` on the You screen, driven by the same field. That field is the gate's own answer (`verification.is_exempt`), not the raw column, so an exempt account is never nagged. ⇄ iOS `APIError.verificationRequired` — which also fixes a real bug: iOS collapsed EVERY 403 into `.unauthorized` ("Your session expired"), so the gate would have told people to sign out of a working account. Web and Android additionally treat these as non-queueable, because a refusal retried by an offline queue is worse than one shown; iOS has no write queue. No iOS SCREEN renders it yet — the message is correct, the affordance is not there. Until `SMTP_HOST` is set the gate is inert |
