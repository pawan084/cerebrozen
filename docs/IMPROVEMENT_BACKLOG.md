...
23. ✅ **DONE** **[safety]** Add repeated AI-disclosure: banner on first coach message and again every N turns / 3 hours of use (CA SB243). `M` — a quiet system line in the transcript (never a bubble, never a modal): once at the head of a session, then again on the engine's **`pause` crossing**. The cadence deliberately reuses the `pacing` marker shipped for CHAT_SPEC #7 rather than a client counter — the crossing is derived server-side from checkpointed history, so it survives a process death and **both clients disclose at the same points in the same conversation** instead of each drifting. It also lands at the right beat: the engine has just told the coach to offer a break, so a step-back line reads as candour rather than interruption. "Every 3 hours" is the minor-protection form of the rule and we are 18+ gated (server-enforced `adult` claim); turn-count is the proxy, and it is the one we already compute. **`apps/app` had NO in-conversation disclosure at all** — the welcome dialog said it once on day one and the transcript never did, so an employee who onboarded in March and opened the coach in November was told nothing. Both clients now carry it, from one shared sentence.
24. ✅ **DONE (was already true; now guarded)** **[safety]** Add an explicit non-medical disclaimer to the coach composer and onboarding ("not therapy, does not diagnose/treat"). `S` — onboarding already carried it (`ob_cant_do_body`: "Diagnose, prescribe, replace therapy, or handle emergencies") and so did the composer's disclosure sheet ("It isn't medical care and never diagnoses or prescribes"), so **nothing needed writing** — a fourth copy of the same sentence makes all four easier to ignore. What was missing was a guard: `DisclosureCopyTest` now fails if no string tells the user this is not medical care, or if none distinguishes the coach from therapy. Asserts the *substance*, not a wording, so rewriting the copy stays free and deleting the meaning does not. This is the same failure mode that let "AI companion" into the compliance pill — one edit at a time, with nothing watching.
