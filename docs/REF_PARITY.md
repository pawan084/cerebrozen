...
|---|---|---|---|---|---|
| 1 | **Crisis screen in the coach** | `apps/app` | `lib/oracle.ts`, `chat/page.tsx:76,146` | The engine already detects a crisis, takes over deterministically, and streams a scripted reply — and the web client renders it **as ordinary chat text**. `safety_flag` ships on the `done` frame; `crisis_region` is resolved on `/users/me`; `/v1/safety/helplines` is deliberately ungated *specifically to back this screen*. PRODUCT.md matrix: crisis takeover = v1 ✔. | Wiring |
| 2 | **Export + delete as product functions** | `apps/app` | `account/page.tsx:117-131,159` | `GET /v1/privacy/me/export` and `DELETE /v1/privacy/me` are built and tested; the client never calls them. The marketing site **sells** "deletion is a product function" — a rule-6 claim with no client mechanism. | Wiring |
