...
Every backend seam above is documented in `backend/.env.example`.

## 8. Submission automation (fastlane)
...
- `.github/workflows/testflight.yml` runs `fastlane beta` on manual dispatch,
  gated on the ASC secrets (no-op safe until you add them).

Local: `cd CereBro && bundle install && bundle exec fastlane ios beta` after
exporting the three `ASC_*` env vars.
