...
| **Testing the edge** | — | The e2e stack runs a `caddy` service on the PRODUCTION `deploy/Caddyfile` (one line added at boot for local certs; `e2e/caddy-testable.sh` diffs the rest and refuses to start if anything else changed), reached over TLS via compose network aliases for each hostname. `e2e/tests/edge-headers.spec.ts` asserts what a client finally receives, which is where two shipped header bugs were found — see docs/TODO.md, 2026-08-23 |
